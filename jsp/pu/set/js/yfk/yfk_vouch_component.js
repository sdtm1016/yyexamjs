...
function queryButtonClick(btn){
	...
	//部门选择
		...
		window.parent.openWindow('basic_comref_dptref','pu_set_yfk_vouchSet',param);
	}
	//供应商或者客户
	else if (queryid=="CDWCODE") {
		param.subjectBusinessId = "";
		param.subjectBusinessName = "";
		window.parent.openWindow(dwrefurl,'pu_set_yfk_vouchSet',param);
	}
	//结算科目
	else if (queryid=="CCODE") {
		param.subjectBusinessId = "";
		param.subjectBusinessName = "";
		window.parent.openWindow('dsign_subjectsreference','pu_set_yfk_vouchSet',param);
	}
	...
	//业务员
		...
		window.parent.openWindow('basic_comref_personref','pu_set_yfk_vouchSet',param);
	}
	//结算方式
	else if (queryid=="CSSCODE") {
		param.subjecPersonId = "";
		param.subjecPersonName = "";
		window.parent.openWindow('basic_comref_settlestyleGridRef','pu_set_yfk_vouchSet',param);
	}
	//币种
	else if (queryid=="CEXCH_NAME") {
		param.subjecPersonId = "";
		param.subjecPersonName = "";
		window.parent.openWindow('basic_comref_exchref','pu_set_yfk_vouchSet',param);
	}
	
	//到期日期
	else if (queryid=="CDEFINE4") {
		WdatePicker({
		      el:"CDEFINE4",
		      position:{left:100,top:10},
		      onpicking:function(dp){
...
function deliverValue(valueObject){
	...
	if(focusArea=="topArea"){
		...
		//摘要参照
		else if (queryid=="CDIGEST") {
			//显示名称
			document.getElementById(queryid).value =  valueObject.selBdigestName;
			//存储代码
			document.getElementById(queryid).setAttribute("dbvalue", valueObject.selBdigestCode);
...
function tableCellWindowMaper(title){
	...
	case "摘要":
		param.subjecPersonId = "";
		param.subjecPersonName = "";
		window.parent.openWindow('dsign_reference','pu_set_yfk_vouchSet',param);
