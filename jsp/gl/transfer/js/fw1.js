...
function queryFunctionGrid(){
	$.ajax({
		url:"glFunction!queryFunctionList.action",
		type:"post",
		datatype:"json",
...
function showFunctionGrid(functionList) {
	...
	for (var i = 0; i<length; i++) {
		...
		$("#left_select")[0].options.add(new Option(strNullProc(funcDesc),i,false,false));
		$("#right_select")[0].options.add(new Option(strNullProc(funcName),i,false,false));
	}	
}
