		...
		tds.each(function (){
			if($(this).attr("md")=="0"){
				...
				if($(this).html()==""||$(this).html()==null){
					...
					if($(this).attr("name")=="accass.mb"||$(this).attr("name")=="accass.mbF"||$(this).attr("name")=="accass.NbS"){
		...
		if(f){
			...
			$(row).find("td").each(function(){
				...
				if(name!=""&&name!=null&&typeof(name)!="undefined"){
					if(name=="accass.mb"||name=="accass.mbF"||name=="accass.nbS"){
...
function onWindowClose(){
	...
	if(mark.length==1){
		...
		if(mar!="1"){
			...
			if(mar!="1"){
				if(result!=null){
					window.parent.getParentWindow('gl_set_operat').setData(result);
				}
				window.parent.justCloseWindow('gl_set_operat');
				return true;
			}
		}else{
...
$(document).ready(function(){
	...
	var param = window.parent.valueMap.get("gl_set_operat");
	...
	mtd=mtd+"<td md='1' onclick='selectTd(this)' class='mtd' name='accass.cbegindC' style='text-align:center'>"+$(cell).parent().children()[2].innerHTML+"</td><td md='1' name='accass.mb' class='mtd' onclick='selectTd(this)' ></td>";
	var ntype=$(cell).attr("ntyp");
	if(ntype=="1"){
		th=th+"<td width='90px'>外币</td>";
		mtd=mtd+"<td md='1' name='accass.mbF' class='mtd' onclick='selectTd(this)' ></td>";
	}else if(ntype=="2"){
		th=th+"<td width='90px'>数量</td>";
		mtd=mtd+"<td md='1' name='accass.nbS' class='mtd' onclick='selectTd(this)' ></td>";
	}else if(ntype=="3"){
		mtd=mtd+"<td md='1' name='accass.mbF' class='mtd' onclick='selectTd(this)' ></td>";
		mtd=mtd+"<td md='1' name='accass.nbS' class='mtd' onclick='selectTd(this)' ></td>";
		th=th+"<td width='90px'>外币</td>";
		...
		success: function(data){
			...
			if(accasslist!=null){
			   for(var i=0;i<accasslist.length;i++){
				   ...
				   str=str+"<td onclick='selectTd(this)' class='mtd' name='accass.mb'>"+donumswitch(transFloat(temp.mb))+"</td>";
				   if(ntype=="1"){
					   str=str+"<td onclick='selectTd(this)' class='mtd' name='accass.mbF'>"+donumswitch(transFloat(temp.mbF))+"</td>";
					}else if(ntype=="2"){
						str=str+"<td onclick='selectTd(this)' class='mtd' name='accass.nbS'>"+donumswitch(transFloat(temp.nbS))+"</td>";
					}else if(ntype=="3"){
						str=str+"<td onclick='selectTd(this)' class='mtd' name='accass.mbF'>"+donumswitch(transFloat(temp.mbF))+"</td>";
						str=str+"<td onclick='selectTd(this)' class='mtd' name='accass.nbS'>"+donumswitch(transFloat(temp.nbS))+"</td>";
					}
...
function queryButtonClick(btn){
	...
	case "部门":
		window.parent.openWindow('basic_comref_dptref','gl_set_operat');
		break;
	case "个人":
		window.parent.openWindow('basic_comref_personref','gl_set_operat');
		break;
	case "客户":
		window.parent.openWindow('basic_comref_cusref','gl_set_operat');
		break;
	case "供应商":
		window.parent.openWindow('basic_comref_supref','gl_set_operat');
		break;
	case "业务员":
		window.parent.openWindow('basic_comref_personref','gl_set_operat');
		break;
	case "项目":
		if(cassItem!=null){
			...
			window.parent.openWindow("basic_comref_fitemref","gl_set_operat",itemParam);
			//deliverValue("eee");
		}else{
