...
$(document).ready(function(){
	//setEntryDate();
	var t_param=window.parent.valueMap.get("fa_card_entry");
	smark=t_param.smark;
	...
	if(smark!="2"){
		...
		$.ajax({
			...
			success: function(data){
				...
				$("#yzjl").html(numberround(facard.dbldeprrate,4));
...
function findAssettypes(){//查询资产类别，初始化数据
	...
	var par=window.parent.valueMap.get("fa_card_entry");
...
function faCardEdit(event,div){//表格编辑控制
	if(smark=="0"){
		return;
	}
	...
	if(div.getElementsByTagName("input").length==0){
		...
		case "bmmc":
			...
			var component = "<input type='button' value='部门名称' style='width:88px;height:20px;margin-left:-92px;margin-top:-4px;' onclick='window.parent.openWindow(\"fa_basic_ref_dr\",\"fa_card_entry\");tagid=\"sdeptnum\";tagname=\"re_depart\";'/><input onfocus='dofocustoenter(\"FaDepartments\",this,\"tagid\",\"sdeptnum\",\"tagname\",\"re_depart\")' id='re_depart' type='text' value='"+temp+"'  style='height:14px;width:280px;margin-top:-4px;'/>";
			div.innerHTML=component;
			div.getElementsByTagName("input")[1].select();
			//cursorToEnd(div.getElementsByTagName("input")[1]);
			currentEditingElement=div;
		...
		case "zjfs":
			...
			var component = "<input type='button' value='增加方式' style='width:88px;height:20px;margin-left:-92px;margin-top:-4px;' onclick='window.parent.openWindow(\"fa_basic_ref_amr\",\"fa_card_entry\");tagid=\"sorgaddid\";tagname=\"re_am\";'/><input onfocus='dofocustoenter(\"FaOrigins\",this,\"tagid\",\"sorgaddid\",\"tagname\",\"re_am\")' id='re_am' type='text' value='"+temp+"'  style='height:14px;width:90px;margin-top:-4px;'/>";
			div.innerHTML=component;
			div.getElementsByTagName("input")[1].select();
			//cursorToEnd(div.getElementsByTagName("input")[1]);
			currentEditingElement=div;
		...
		case "syzk":
			...
			var component = "<input type='button' value='使用状况' style='width:88px;height:20px;margin-left:-92px;margin-top:-4px;' onclick='window.parent.openWindow(\"fa_basic_ref_usr\",\"fa_card_entry\");tagid=\"sstatusid\";tagname=\"re_usr\";'/><input onfocus='dofocustoenter(\"FaStatus\",this,\"tagid\",\"sstatusid\",\"tagname\",\"re_usr\")' id='re_usr' type='text' value='"+temp+"' style='height:14px;width:90px;margin-top:-4px;'/>";
			div.innerHTML=component;
			div.getElementsByTagName("input")[1].select();
			//cursorToEnd(div.getElementsByTagName("input")[1]);
			currentEditingElement=div;
		...
		case "zjff":
			...
			var component = "<input type='button' value='折旧方法' style='width:88px;height:20px;margin-left:-92px;margin-top:-4px;' onclick='window.parent.openWindow(\"fa_basic_ref_dmr\",\"fa_card_entry\");tagid=\"sdeprmethodid\";tagname=\"re_dmr\";'/><input readonly='readonly' id='re_dmr' type='text' value='"+temp+"' onblur='' style='height:14px;width:90px;margin-top:-4px;'/>";
			div.innerHTML=component;
			div.getElementsByTagName("input")[1].select();
			//cursorToEnd(div.getElementsByTagName("input")[1]);
			currentEditingElement=div;
		...
		case "dyzjkm":
			div.style.overflow="";
			var component = "<input type='button' value='对应折旧科目' style='width:88px;height:20px;margin-left:-92px;margin-top:-4px;' onclick='window.parent.openWindow(\"dsign_subjectsreference\",\"fa_card_entry\");tagid=\"re_code\";tagname=\"re_code\";'/><input onfocus='dofocustoenter(\"code\",this,\"tagid\",\"re_code\",\"tagname\",\"re_code\")'  id='re_code' type='text' value='"+temp+"' onblur='' style='height:14px;width:90px;margin-top:-4px;'/>";
			div.innerHTML=component;
			div.getElementsByTagName("input")[1].select();
			//cursorToEnd(div.getElementsByTagName("input")[1]);
			currentEditingElement=div;
		...
		//$(div).find("input[type='text']")[0].select();
		//cursorToEnd($(div).find("input[type='text']")[0]);
	}
		...
		if(currentEditingElement.tagName=="TD" || (currentEditingElement.tagName!="TD" && evtsrc.parentNode!=currentEditingElement.parentNode)){
			if(evtsrc.tagName!="INPUT" || (evtsrc.tagName=="INPUT" && evtsrc.parentNode.parentNode.className=="group")){
				...
				if(currentEditingProperty=="synx"){
					...
					var str = inputs[0].value +"年"+ inputs[1].value +"月";
					currentEditingElement.innerHTML = str;
				}else if(currentEditingProperty=="syzk"){//如果是使用状况，需要判断是否进行折旧判断
					var bned=$("#syzk").attr("bneed");
					if(bned=="0"){
						$("#yzjl").html("0.00");
						$("#yzje").html("0.00");
				...
				if(currentEditingProperty=="synx"||currentEditingProperty=="zjff"||currentEditingProperty=="yz"||currentEditingProperty=="jczl"||currentEditingProperty=="jcz"||currentEditingProperty=="ljzj"||currentEditingProperty=="jz"){//如果是使用状况，需要判断是否进行折旧判断
					var bned=$("#syzk").attr("bneed");
					if(bned=="0"){
						$("#yzjl").html("0.00");
						$("#yzje").html("0.00");
...
function dostartdate(){
	...
	if((nowtime[0]-0)<(starttime[0]-0)||((nowtime[0]-0)==(starttime[0]-0)&&(nowtime[1]-0)<(starttime[1]-0))){
		$("#my97DatePickerField").val("");
		$("#yjtyf").html("0");
		jAlert("卡片的开始日期不能在本月或在本月之后");
		var bned=$("#syzk").attr("bneed");
		if(bned=="0"){
			$("#yzjl").html("0.00");
			$("#yzje").html("0.00");
		...
		var ntime=(nowtime[0]-starttime[0])*12+(nowtime[1]-starttime[1])-1;
		if(ntime<0){
			$("#my97DatePickerField").val("");
			$("#yjtyf").html("0");
			jAlert("原始卡片的开始日期不能在本月或在本月之后");
			var bned=$("#syzk").attr("bneed");
			if(bned=="0"){
				$("#yzjl").html("0.00");
				$("#yzje").html("0.00");
