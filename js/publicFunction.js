			...
			if (d_result != "") {
				...
				if (ress[0] == "0") {
					if (type == "code") {
						...
						param.ccode = ress[2];
						param.cname = ress[3];//
					}else if (type == "Inventory") {// 存货档案
						//id/存货编码 /存货名称/税率/计量单位/规格型号
						param.id = ress[1];
						param.ccode = ress[2];
						param.cname = ress[3];//Inventory
						param.itaxrate=ress[4];
						param.cinvmUnit=ress[5];
						param.cinvstd=ress[6];
...
/*** 
 * 加密、解密算法： 
 * 加密：base64encode(utf16to8(value))
 * 解密：utf8to16(base64decode(value))
 */
